...
public class PhysicalAppointmentBookingController {
    ...
    @Autowired
    public PhysicalAppointmentBookingController(NavBarAuthInfoChangeHelper navBarAuthInfoChangeHelper, AppUserService appUserService, AppointmentService appointmentService, EmailSender emailSender) {
    ...
    @GetMapping("/booking")
    public String showPhysicalBookingPage(Model model, Date chooseDate, String fullName,
                                          String phone, String email) throws ParseException {

        String template =  "physical_appointment_booking_page.html";
        model.addAttribute("appointment_form", new AppointmentForm(fullName, phone, email, chooseDate, ""));
        model.addAttribute("availableSlots", appointmentService.getAvailableTimeSlots(chooseDate, AppUserAppointmentType.PHYSICAL.toString()));
    ...
    public String saveBooking(@ModelAttribute("appointment_form") @Valid AppointmentForm appointmentForm,
        ...
        if (appointmentForm.getDescription().equals("-")){
            ...
            return showPhysicalBookingPage(model, currentDate, fullName, phone, email);
        }
        ...
        if (result.hasErrors()) {
            ...
            return "physical_appointment_booking_page.html";
        }
        ...
        emailSender.send(appointmentForm.getEmail(),
                buildAppointmentInfoEmail(appointmentForm.getFullName(), appointmentDatetime),
                "Appointment confirmed");


        return "redirect:/physicalAppointment/bookingSuccessful";
    }



    private String buildAppointmentInfoEmail(String name, String datetime) {
        return "<div style=\"font-family:Helvetica,Arial,sans-serif;font-size:16px;margin:0;color:#0b0c0c\">\n" +
                ...
                "        \n" +
                "            <p style=\"Margin:0 0 20px 0;font-size:19px;line-height:25px;color:#0b0c0c\">Dear " + name + ",</p><p style=\"Margin:0 0 20px 0;font-size:19px;line-height:25px;color:#0b0c0c\"> Your appointment at akVirtualClinic scheduled for " + datetime + " has been confirmed.\n"  +
                " Thank you for choosing our clinic.</p><blockquote style=\"Margin:0 0 20px 0;border-left:10px solid #b1b4b6;padding:15px 0 0.1px 15px;font-size:19px;line-height:25px\"><p style=\"Margin:0 0 20px 0;font-size:19px;line-height:25px;color:#0b0c0c\"></p></blockquote>\n <p>See you soon.</p>" +
                "        \n" +
                "      </td>\n" +
                "      <td width=\"10\" valign=\"middle\"><br></td>\n" +
                "    </tr>\n" +
